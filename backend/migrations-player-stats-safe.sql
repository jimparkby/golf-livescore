-- Safe migration for player statistics
...
-- Indexes for player_tournament_results (skip if exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_tournament_results_player') THEN
    CREATE INDEX idx_player_tournament_results_player ON player_tournament_results(player_name);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_tournament_results_tournament') THEN
    CREATE INDEX idx_player_tournament_results_tournament ON player_tournament_results(tournament_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_tournament_results_place') THEN
    CREATE INDEX idx_player_tournament_results_place ON player_tournament_results(place);
  END IF;
END $$;

-- Table for player statistics aggregation
CREATE TABLE IF NOT EXISTS player_statistics (
  ...
  estimated_hcp DECIMAL(4, 1),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Indexes for player_statistics (skip if exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_statistics_name') THEN
    CREATE INDEX idx_player_statistics_name ON player_statistics(player_name);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_statistics_first_places') THEN
    CREATE INDEX idx_player_statistics_first_places ON player_statistics(first_places DESC);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_statistics_hcp') THEN
...
-- Indexes for player_nominations (skip if exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_nominations_player') THEN
    CREATE INDEX idx_player_nominations_player ON player_nominations(player_name);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_player_nominations_tournament') THEN
