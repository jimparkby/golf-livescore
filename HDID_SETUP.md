...
В профиле пользователя есть кнопка **"Синхронизировать HCP с HDID"**:

1. Ищет пользователя в whitelist по имени/фамилии из профиля
2. Обновляет HCP на значение из HDID
3. Сохраняет timestamp последней синхронизации
4. Показывает успешное сообщение с новым HCP

Это **опциональная** функция — пользователь сам решает, когда синхронизировать HCP.
