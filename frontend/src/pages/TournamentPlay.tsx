...
const Leaderboard = ({
  ...
  /* ── Match Play (2 players or 2 teams) ── */
  if (isMatchPlay) {
    ...
    const holeResults: ("p1" | "p2" | "halved" | null)[] = holes.map((h) => {
      ...
      if (s1.score < s2.score) { p1Wins++; return "p1"; }
      if (s2.score < s1.score) { p2Wins++; return "p2"; }
      return "halved";
    });
    ...
    return (
      <div className="flex-1 overflow-y-auto px-5 pb-4 pt-2 space-y-3">
        <div className="rounded-2xl p-4 text-center" style={{ background: "#1a1a1a" }}>
          <div className="text-white/40 text-xs uppercase tracking-wider mb-1">Match status</div>
          ...
          {holes.map((h, i) => {
            ...
            return (
              <div key={h.number} className="grid items-center px-4 py-2 border-t border-white/5" style={{ gridTemplateColumns: "1fr auto auto 1fr" }}>
                <div className={cn("font-black text-lg tabular-nums", r === "p1" ? "text-action" : "text-white/50")}>{s1?.score ?? "—"}</div>
                ...
                <div className={cn("font-black text-lg tabular-nums text-right", r === "p2" ? "text-action" : "text-white/50")}>{s2?.score ?? "—"}</div>
              </div>
            );
          })}
        </div>
  ...
  /* ── Team leaderboard (Scramble / Fourball) ── */
  if (isTeam && teams) {
    type TeamEntry = { name: string; playerIds: string[]; total: number; vsPar: number; points: number; holesPlayed: number };
    const teamEntries: TeamEntry[] = [
      ...
      let total = 0, vsPar = 0, points = 0, holesPlayed = 0;

      if (isScramble) {
        // Score stored under captain only
        ...
        // Fourball: best score per hole among team members
        ...
        holesSet.forEach((holeNum) => {
          ...
          const par = h?.par ?? 4;
          const teamScores = teamPlayers.map((p) => activeRound.scores[p.id]?.find((s) => s.hole === holeNum)?.score).filter((s) => s !== undefined) as number[];
          if (teamScores.length > 0) {
            const best = Math.min(...teamScores);
            total += best;
            vsPar += best - par;
            holesPlayed++;
          }
      ...
      return { name, playerIds, total, vsPar, points, holesPlayed };
    });

    const sorted = [...teamEntries].sort((a, b) =>
      isStableford ? b.points - a.points : a.vsPar - b.vsPar
    );
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4 pt-2">
        <div className="rounded-2xl overflow-hidden" style={{ background: "#1a1a1a" }}>
          <div className="grid px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider" style={{ gridTemplateColumns: "2rem 1fr auto auto", color: "rgba(255,255,255,0.4)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div className="w-12 text-center">{isStableford ? "Pts" : "±"}</div>
          ...
          {sorted.map((e, i) => (
            <div key={e.name} className="grid items-center px-4 py-3 border-t border-white/5" style={{ gridTemplateColumns: "2rem 1fr auto auto" }}>
              ...
              <div className="w-12 text-center font-black text-base tabular-nums" style={{ color: isStableford ? "#fff" : parColor(e.vsPar) }}>
                {isStableford ? e.points : parSign(e.vsPar)}
              </div>
          ...
          <div key={e.player.id} className="grid items-center px-4 py-3 border-t border-white/5" style={{ gridTemplateColumns: "2rem 1fr auto auto" }}>
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-white/40 text-xs">{e.holesPlayed} holes</div>
              </div>
            </div>
            <div className="w-12 text-center">
              {isStableford
                ? <span className="text-white font-black text-base tabular-nums">{e.points}</span>
                : <span className="font-black text-base tabular-nums" style={{ color: parColor(e.vsPar) }}>{parSign(e.vsPar)}</span>
              }
            </div>
            <div className="w-12 text-center text-white/60 font-bold text-sm tabular-nums">{e.total || "—"}</div>
          </div>
...
const TournamentRoundPlayer = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "#0a0a0a" }}>
      {/* Header */}
      <div className="flex items-center justify-between px-5" style={{ paddingTop: "max(env(safe-area-inset-top), 14px)", paddingBottom: 10 }}>
